...
paises = FuzzyWordCompleter(paises)

continentes = []
for i in countries:
    if i["continent"] not in continentes:
        continentes.append(i["continent"])

continentes = FuzzyWordCompleter(continentes)
...
def complete(o):
    if o == "paises":
        pais_cont = []
        text = prompt(
            "garu@autocompleter[pais] > ", completer=paises, complete_while_typing=True
        )
        pais_cont.append(text)
        for i in countries:
            if text == i["name"]:
                pais_cont.append(i["continent"])
        return pais_cont
    elif o == "continentes":
        text = prompt(
            "Continente: ", completer=continentes, complete_while_typing=True
        )
        return text
        ...
        lista_bandas = []
        for i in x:
            lista_bandas.append(i["Nombre"])

        lista_bandas = FuzzyWordCompleter(lista_bandas)
        
        text = prompt("garu@autocompleter[gang] > ", completer=lista_bandas, complete_while_typing=True)
        return text
    elif o == "modulos":
        n = "modulos_disp.txt"
        ms = []

        system("wget http://10.100.10.90:1111/%s"%n)
        system("clear")
        #print("OK")
        txt = open(n, "r")
        for i in txt:
        ...
        txt.close()
        os.remove(n)
        ms = FuzzyWordCompleter(ms)
        text = prompt("garu@autocompleter[modulo] > ", completer=ms, complete_while_typing=True)
        return text
